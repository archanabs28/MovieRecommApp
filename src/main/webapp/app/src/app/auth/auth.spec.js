describe('Auth Section >',function(){
    ...
    it('check if user exists', function () {
        ...
        $httpBackend.flush();
        expect(new_user.emailId.split('@')[0]).toBe(userName);
    });

    //Test 2
    it('should Login a dummy user and check if isLoggedIn() is being set to true', function () {
    ...
    //Test 3
    it('should check if isLoggedIn is true, then Logout and check if the value of isLoggedIn() is set to false', function () {
        spyOn(mocksessionService, 'isLoggedIn').and.callThrough();
        var isLoggedIn = mocksessionService.isLoggedIn();
        expect(mocksessionService.isLoggedIn).toHaveBeenCalled();
        expect(isLoggedIn).toBeTruthy();
    ...
    //Test 4
    it('should login a user and check if user is admin', function () {
    ...
    //Test 5
    it('should login a user and check if getLoggedInUser is working', function () {
        var user = testData.user();
        spyOn(mocksessionService,'login').and.callThrough();
