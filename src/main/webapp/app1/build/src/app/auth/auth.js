angular.module( 'ngBoilerplate.auth', ['ui.router','ngResource'])
    ...
    .factory('sessionService',function () {
        ...
        session.isAdmin = function(){
            try {
                ...
                if(user.type.search('admin')!=-1){return true;}
                else{return false;}
            }catch (err){
                return false;
            }
    ...
    .factory('userExists',["$resource", function ($resource) {
        return $resource('/user/email/:email',{email:'@email'});
    }])

    .controller( 'LoginCtrl', ["$scope", "sessionService", "userExists", "$state", function LoginCtrl( $scope,sessionService,userExists,$state ) {
    ...
    .controller( 'RegisterCtrl', ["$scope", "$state", "sessionService", "accountService", "userExists", function RegisterCtrl( $scope, $state, sessionService, accountService, userExists ) {
